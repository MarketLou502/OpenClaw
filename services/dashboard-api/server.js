  ...
  while (d < endExclusive) {
    for (const e of expenses) {
      if (d.getDate() === e.due_day) {
        out.push({ id: e.id, name: e.name, amount: e.amount, due_date: toYMD(d), is_negotiable: !!e.is_negotiable });
      }
    }
...
async function handle(req, res) {
  ...
  try {
    ...
    if (resource === 'grocery') {
      ...
      if (a && a !== 'history' && req.method === 'DELETE') {
    ...
    if (resource === 'health') {
      ...
      if (a === 'foods' && b && !c && req.method === 'PATCH') {
        ...
        patchHealthState({
          calories: { current: outcome.result.totals.calories },
          protein: { current: outcome.result.totals.protein },
        });
