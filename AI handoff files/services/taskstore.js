...
function addItem(filePath, text) {
  ...
  const item = { id: newId(), text: String(text).trim(), done: false, added: todayStr(), doneDate: null };
  items.push(item);
  writeItems(filePath, items);
  return item;
}

function toggleItem(filePath, id, done) {
  const items = readItems(filePath);
...
function readTodayHealth(todayStateFile, defaults) {
  return ensureToday(todayStateFile, (today) => ({ date: today, ...defaults }));
}
...
module.exports = {
  ...
  readItems, writeItems, addItem, toggleItem,
